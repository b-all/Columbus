...
(function() {
  ...
  module.exports = GraphDatabase = (function() {
    ...
    function GraphDatabase(opts) {
      ...
      this.url = opts.url, this.headers = opts.headers, this.proxy = opts.proxy;
      if (!this.url) {
    ...
    GraphDatabase.prototype.cypher = function(opts, cb, _tx) {
      var body, commit, format, formats, headers, method, params, path, queries, query, raw, rollback, single;
      if (opts == null) {
        opts = {};
      }
      if (typeof opts === 'string') {
      ...
      queries = opts.queries, query = opts.query, params = opts.params, headers = opts.headers, raw = opts.raw, commit = opts.commit, rollback = opts.rollback;
      if (!_tx && rollback) {
      ...
      if (queries && raw) {
        throw new TypeError('When batching multiple queries, `raw` must be specified with each query, not globally.');
      }
      method = 'POST';
      if (rollback) {
        method = 'DELETE';
      ...
      body = {
        statements: (function() {
          ...
          for (_i = 0, _len = queries.length; _i < _len; _i++) {
            ...
            _ref = query, query = _ref.query, params = _ref.params, raw = _ref.raw;
            formats.push(format = raw ? 'row' : 'rest');
            _results.push({
              statement: query,
              parameters: params || {},
      ...
      return this.http({
        ...
        return function(err, body) {
          var columns, data, error, errors, i, result, results;
          if (err) {
            return cb(err);
          }
          if (_tx != null) {
          ...
          results = body.results, errors = body.errors;
          results = (function() {
